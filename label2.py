# %% [markdown]
# # Feature Engineering for Label 2
...
# %% [markdown]
# label 2 has missing values. We cannot give a dataset with missing target labels to a classifier. So we need to remove the rows with missing labels.

# %% [markdown]
# ### Split train and validate data for features and labels

# %%
x_train = df_train[df_train['label_2'].notna()].values[:, 0:256]
y_train = df_train[df_train['label_2'].notna()].values[:, 257]

x_valid = df_valid.values[df_valid['label_2'].notna()][:, 0:256]
y_valid = df_valid.values[df_valid['label_2'].notna()][:, 257]

# %%
new_missing_values = pd.DataFrame(x_train, columns=FEATURES).isna().sum()
new_missing_values[new_missing_values > 0]

# %%
new_missing_values = pd.DataFrame(y_train).isna().sum()
new_missing_values[new_missing_values > 0]
...
# %%
pd.DataFrame(x_train, columns=FEATURES).shape
...
# %% [markdown]
# ### Accuray before Feature Engineering
...
# %%
pca_for_kbest = PCA(n_components=0.98)
...
# %%
df_for_csv.to_csv('190239A_label2.csv', index=False)
